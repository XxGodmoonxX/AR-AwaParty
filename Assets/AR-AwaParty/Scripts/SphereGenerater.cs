...
namespace UnityEngine.XR.iOS {
	public class SphereGenerater : MonoBehaviour {
      ...
      sphere.GetComponent<Renderer>().material = material;
			...
			sphere.transform.position = ballPrefab.transform.TransformPoint(0, 0, 0);
      ...
      //sphereにはRigidbodyを持たせて重力を与えておかないと、床の上には配置されないので注意が必要。Rigidbodyで重力を持たせないとCubeは宙に浮いた状態になる
      sphere.AddComponent<Rigidbody>();

			sphere.GetComponent<Rigidbody>().AddForce(0, 0, 0);

      // sphere.GetComponent<Rigidbody>().AddForce(sphere.transform.TransformDirection(0,0,0),ForceMode.Impulse);
